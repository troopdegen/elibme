#!/usr/bin/env bash
# Install the elibme skill into whichever agent harness you use.
#
# One canonical file — skills/elibme/SKILL.md — is the source of truth.
...
SKILL_SRC="$SOURCE_DIR/skills/elibme/SKILL.md"
MARKER_BEGIN="<!-- BEGIN elibme (managed by install.sh — edits here are overwritten) -->"
MARKER_END="<!-- END elibme -->"
...
Targets:
  claude     ~/.claude/skills/elibme/SKILL.md          (default)
  cursor     ./.cursor/rules/elibme.mdc                (project-scoped)
...
Flags:
  ...
  --build      regenerate dist/ from skills/elibme/SKILL.md — for
...
remove_managed_block() {
  ...
  if ! grep -qF "$MARKER_BEGIN" "$file"; then
    echo "Skipped $file: no managed elibme block found."
    return 0
  fi
  tmp="$(mktemp)"
  strip_managed_block "$file" > "$tmp"
  mv "$tmp" "$file"
  echo "Removed the elibme block from: $file"
}
...
install_claude() {
  ...
  local dest="$skills_dir/elibme"

  if [ "$UNINSTALL" = "1" ]; then
    if [ -f "$dest/.elibme-managed" ]; then
  ...
  if [ -e "$dest" ] && [ ! -f "$dest/.elibme-managed" ]; then
...
install_cursor() {
  local dest="./.cursor/rules/elibme.mdc"
  ...
  if [ -f "$dest" ] && ! grep -qF "elibme" "$dest"; then
    confirm_overwrite "$dest" || { echo "  Skipped."; return 0; }
  fi
  ...
  echo "Installed: $dest"
  echo "Cursor loads this on request — reference it with @elibme, or let the"
